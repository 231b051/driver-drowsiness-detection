...
async function startCamera() {
  try {
    ...
    // Call real backend every 1.5 seconds
...
let yawnCount = 0;
let lastAlertTime = 0;
const ALERT_COOLDOWN = 8000;   // 8 seconds between alerts
const CLOSED_THRESHOLD = 3;    // eyes closed 3 times in a row = alert
const YAWN_THRESHOLD = 2;      // yawn 2 times in a row = alert
...
async function sendFrameToBackend() {
  ...
  try {
    ...
    if (label === 'NEUTRAL') {
      ...
      // Only alert after CLOSED_THRESHOLD consecutive detections
      ...
      // Only alert after YAWN_THRESHOLD consecutive detections
